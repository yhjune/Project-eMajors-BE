...
dm_pdf_path = root+'major_origin/인공지능대학/복수전공_인공지능학과.pdf'
...
def revise_double_major_df(dm_pdf_path):
    df = tabula.read_pdf(dm_pdf_path, pages="all",lattice=True, area=[163.5, 13,802.1,580.5]) # read pdf (head include)
    df_concat = pd.concat(df, ignore_index=True).dropna(thresh=2) # concat dataframe, drop rows na < 4 
...
df_list, df_condi = revise_double_major_df(dm_pdf_path)

df_list.to_csv(output_path+"condi.csv",mode='w')